...
class CarAgent(Agent):  # Car
    ...
    def move(self):
        ...
        possible = (
            (self.locationX + 1, self.locationY),
            (self.locationX - 1, self.locationY),
            (self.locationX, self.locationY + 1),
            (self.locationX + 1, self.locationY + 1),
        )
        if self.locationY >= 19:
            possible = (
                (self.locationX + 1, self.locationY),
                (self.locationX - 1, self.locationY),
                (self.locationX, (self.locationY + 1) % 20),
                (self.locationX + 1, (self.locationY + 1) % 20),
            )

        if self.locationX >= 19:
            possible = (
                ((self.locationX + 1) % 20, self.locationY),
                (self.locationX - 1, self.locationY),
                (self.locationX, self.locationY + 1),
                ((self.locationX + 1) % 20, self.locationY + 1),
            )
        ...
        if self.locationX >= 19 and self.locationY >= 19:
            possible = (
                ...
                ((self.locationX - 1) % 20, self.locationY),
                (self.locationX, (self.locationY + 1) % 20),
                ((self.locationX + 1) % 20, (self.locationY + 1) % 20),
            )
        ...
        for neighbor_cell in possible:
            ...
            if len(cellmates) >= 1:
                ...
                if other.agentT == 0:
                    ...
                    possible = tuple(
                        value for value in possible if value != value_to_remove
                    )
...
class CarModel(Model):
    def __init__(self, width, height, num_agents):
        ...
        self.datacollector = mesa.DataCollector(
            {"pasos": "pasos", "choques": "choques", "toretoCrash": "toretoCrash"},
        )
        ...
        for i in range(self.num_agents):
            ...
            self.grid.place_agent(
                cool_agent, (cool_agent.locationX, cool_agent.locationY)
            )
            o += 1  # Contador de ids
